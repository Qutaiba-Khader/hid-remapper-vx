...
- **[`config-tool-web-v2/`](config-tool-web-v2)** — the REDESIGNED tool (v2): vanilla JS, no build step, **combos**, per-field reset-to-default, live monitor. Live at `…/config-tool-web-v2/`.
  ...
  - Tests: `cd config-tool-web-v2 && node --test tests/*.test.js` (**104**, no deps). They include a full end-to-end flow against a fake device (`flow.test.js`), a firmware↔web contract check that reads the firmware source (`contract.test.js`), and guards against the mock's fake data creeping back.
