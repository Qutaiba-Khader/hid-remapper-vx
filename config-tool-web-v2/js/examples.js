...
/* Expose the v1 example library to the v2 tool (72 ready-made configs). The array above is copied
   VERBATIM from config-tool-web/examples.js so it can be re-synced from upstream — the ONLY change
   is that v1's `export default examples;` (an ES module) is dropped, because v2 loads plain
   <script> tags and `export` in a classic script is a SyntaxError that would kill the whole file. */
if (typeof window !== "undefined") window.HRX_EXAMPLES = examples;
